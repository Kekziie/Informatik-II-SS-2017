...
public class SnakeGridGame implements GridGameInterface, ActionListener
{
	...
	/**
	 * The timer controls the game speed 
	 * by iteratively invoking action events - which in turn
	 * result in the execution of one step of the snake. 
	 */
	private Timer timer;
	 ...
	 *  Direction Code: 0 := North;  1 := East; 
	 *                   2 := South; 3 := West
	 */
	private int snakeDirection = 0; 

	/**
	 * The last snake direction encodes the previous movement before a turn.
	 * It is used to prevent movements backwards into the snake's own body.
	 */
	 ...
	 * Constructs the game and starts it immediately.
	 * 
	 * @param inFrame Reference to the frame within this game is played. Needed to register key events and to be able to remove the key listener again at the end of the game. 
	 * @param yourPanel the panle within the game is played. Needed to invoke repaint()
	...
	/**
	 * Closes the game by stopping the time and removing the key listener to the game frame.
	 */
	...
	/**
	 * Initializes the game - especially the grid area, the current snake location, its body, 
	 * resets the points and iteration counters and activates the time to start the snake.
	 */
	private void initFullSnakeGamePanel()
	{
		...
		snakeArea = new Rectangle(gameArea.x+tileSize, gameArea.y+tileSize, gameArea.width-tileSize*2, gameArea.height-tileSize*2);
	...
	/**
	 * A game iteration in which the snakes moves forward in its current heading direction one step.
	 * Invoked currently by an action event, which is periodically invoked when the timer is active. 
	 */
	private void nextStep()
	{
		if(stillAlive  && foodEaten < FOOD_ITEM_GOAL) {
			...
			// game / level done - won or proceed to the next level / or show end-game dialog
			timer.stop();
			// TODO here the next level may be started (you will also need to modify the game re-initalization cf. initFullSnakeGamePanel() )
			if(stillAlive) {
				JOptionPane.showMessageDialog(myPanel, "Congratulations - You won the Game!!!\n"
						+ "After "+iteration+" steps you have reached the maximum \n"
						+ "snake length by eating " + FOOD_ITEM_GOAL + " items.");
			}else{
				JOptionPane.showMessageDialog(myPanel, "Ouch - that hurt :-(\n"
						+ "You stayed alive for "+iteration+" steps\n"
						+ "and have eaten " + foodEaten + " itmes during that time.\n"
								+ "T R Y    A G A I N   !!!");
			}
			initFullSnakeGamePanel();
			myPanel.repaint();
	...
	private void resetFoodLocation()
	{
		...
		while(!done) {
			...
			// TODO - still need to make sure that the food is not put into a barrier.
		}
	}
	...
	/**
	 * Expands the snake by the specified number of segments. 
	 * Initializes the locations of the new segments to the tail of the snake. 
	 * 
	 * @param oldSnake
	 * @param snakeIndex index of the head of the snake in the oldSnake Array.
	 * @param newSnakeLength the new Snake Length
	...
	public void paintFood(Graphics2D g)
	{
		...
		g.fillRect(snakeArea.x + foodLoc[0]*tileSize, snakeArea.y + foodLoc[1]*tileSize, 
				tileSize, tileSize);		
	}
	...
	public void paintSnake(Graphics g)
	{
		...
		for(int i=0; i<snakeLength; i++) { // paint the whole body including the head.
			g.fillRect(snakeArea.x + snakeLoc[i][0]*tileSize, 
					snakeArea.y + snakeLoc[i][1]*tileSize, 
					tileSize, tileSize);		
		}
		// now repaing the head in the proper color.
		g.setColor(snakeHead);
		g.fillRect(snakeArea.x + snakeLoc[snakeIndex][0]*tileSize, 
				snakeArea.y + snakeLoc[snakeIndex][1]*tileSize, 
				tileSize, tileSize);
	}
	 ...
	 * key presses. 
	 * It overwrites the abstract Method keyPressed(KeyEvent e). 
	 * The Method is called when a key is pressed and the frame, within which the game is played, 
	 * is currently listening to the game (via this KeyListener). 
	 */
		...
		public void keyPressed(KeyEvent e)
		{
			...
			case KeyEvent.VK_LEFT:
				if(lastSnakeDirection != 1)
					snakeDirection = 3;
				break;
				// TODO Handle plus = KeyEvent.VK_PLUS/ minus = KeyEvent.VK_MINUS KeyPresses
			default: 
